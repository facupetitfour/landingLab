...
export default function ClientResults({ projectId, initialData }: ClientResultsProps) {
  ...
  useEffect(() => {
    if (initializedId === projectId || !initialData) return;
  ...
  const syncStateWithActionData = (data: any) => {
  ...
  const handleQuickEdit = async (key: string) => {
    ...
    setIsEditing(true);
    try {
      const data = await editCopyAction(projectId, '', key);
      if (data.success) {
        syncStateWithActionData(data);
  ...
  const handleFreeEdit = async () => {
    ...
    setEditInput('');
    try {
      const data = await editCopyAction(projectId, instruction);
      if (data.success) {
        syncStateWithActionData(data);
        showToast('✓ Cambios aplicados');
      }
  ...
  const handleDesignSave = async () => {
    ...
    setIsEditing(true);
    try {
      const data = await updateDesignAction(projectId, editedCopy);
      if (data.success) {
        syncStateWithActionData(data);
